...
main(int argc, char **argv) 
{
	...
	DEBUG("BEFORE SOCKET CREATION...");
	DEBUG("Creating socket...!!!");
	sockfd = mtcp_socket(mctx, AF_INET, SOCK_STREAM, 0);
	DEBUG("AFTER SOCKET CREATION = mtcp_socket...");
	if (sockfd < 0) {
	...
	DEBUG("AFTER SOCKET CREATION = mtcp_setsock_nonblock...");
	if (ret < 0) {
