...
CreateConnection(thread_context_t ctx)
{
  ...
  // {
  //   // printf("adding mtcp_epoll_ctl %d\n", i);
  //   mtcp_epoll_ctl(mctx, ctx->ep, MTCP_EPOLL_CTL_ADD, sockid, &ev);
  // }
...
SendHTTPRequest(thread_context_t ctx, int sockid, struct wget_vars *wv)
{
  ...
  // ctx->stat.writes += wr;
  printf("finished writing in SendHTTPRequest!\n");
...
RunWgetMain(void *arg)
{
  ...
  // while (1)
  {
    fprintf(stderr, "INSIDE WHILE LOOP with core %d!!!!!!!!!\n", core);
    gettimeofday(&cur_tv, NULL);
    ...
    {
      PrintStats();
      fprintf(stderr, "PrintStats\n");
      prev_tv = cur_tv;
    }
    fprintf(stderr, "=================================\n");
    fprintf(stderr, "ctx->pending = %d, ctx->started = %d, ctx->target = %d\n", ctx->pending, ctx->started, ctx->target);
    fprintf(stderr, "=================================\n");
    while (ctx->pending < concurrency && ctx->started < ctx->target)
    {
      if (CreateConnection(ctx) < 0)
    ...
    fprintf(stderr, "BEFORE mtcp_epoll_wait\n");
    ...
    for (i = 0; i < nevents; i++)
    {
      printf("EVENT %d with SOCKETID = %d\n", i, events[i].data.sockid);
      curr_socket_id = events[i].data.sockid;
      ...
      for (int i = 0; i < num_iter; i++)
      {
        memset(buf, 0x90, sizeof(char) * BUF_LEN);
        buf[BUF_LEN - 1] = '\0';
        wr = mtcp_write(ctx->mctx, curr_socket_id, buf, BUF_LEN);
        // while (1)
        // {
        //   wr = mtcp_write(ctx->mctx, events[i].data.sockid, buf, BUF_LEN);
        //   printf("wr=%d\n", wr);
        //   if (wr > 0)
        //   {
        //     break;
