...
CreateConnection(thread_context_t ctx)
{
  ...
  TRACE_INFO("this is ret from mtcp_bind: %d", ret);

  ret = mtcp_listen(mctx, sockid, backlog);
  if (ret < 0)
  {
    TRACE_ERROR("Failed to listen: %s", strerror(errno));
  }
  TRACE_INFO("this is ret from mtcp_listen: %d", ret);
...
static inline int
HandleReadEvent1(thread_context_t ctx, int sockid, struct wget_vars *wv)
{
  mctx_t mctx = ctx->mctx;
  char buf[BUF_SIZE];
  char *pbuf;
...
RunWgetMain(void *arg)
{
  ...
  while (!done[core])
  {
    ...
    // while (1)
    // {
    //   fprintf(stderr,"inside the while loop now!\n");
    //   nevents = mtcp_epoll_wait(mctx, ep, events, MAX_EVENTS, -1);
    //   fprintf(stderr, "=================================\n");
    //   fprintf(stderr, "nevents = %d\n", nevents);
    //   fprintf(stderr, "=================================\n");
    //   fprintf(stderr,"AFTER mtcp_epoll_wait...\n");
    //   fprintf(stderr,"===============================\n");
    //   if (nevents < 0)
    //   {
    //     if (errno != EINTR)
    ...
    //   for (i = 0; i < nevents; i++)
    //   {
    //     fprintf(stderr, "events[i].data.sockid = %d\n", events[i].data.sockid);
    //     fprintf(stderr, "sockid = %d\n", sockid);
    //     fprintf(stderr, "=================================\n");

    //     if (events[i].data.sockid == sockid)
    //     {
    //       c = mtcp_accept(mctx, sockid, NULL, NULL);
    //       fprintf(stderr,"AFTER mtcp_accept...\n");
    //       if (c >= 0)
    //       {
    //         if (c >= MAX_FLOW_NUM)
    //         {
    //           TRACE_ERROR("Invalid socket id %d.", c);
    //         }
    //         TRACE_INFO("Accepted new connection");
    //       }
    //       else
    //       {
    //         TRACE_ERROR("mtcp_accept() %s", strerror(errno));
    //       }
    //       // mtcp_epoll_ctl(mctx, ep, MTCP_EPOLL_CTL_DEL, sockid, &ev);
    //       // sockid = c;
    ...
    //     else
    //     {
    //       TRACE_ERROR("Received event on unknown socket.");
    //     }
    //   }
    // }
    ...
    // while (1) {
    //   for (int i = 1; i <= 3; i++) {
    //     printf("MTCP_EPOLLIN in WHILE for socket id = %d\n", i);
    //     HandleReadEvent(ctx, i, &wvars[i]);
    //   }
    // }
    ...
    for (i = 0; i < nevents; i++)
    {
      ...
      c = mtcp_accept(mctx, events[i].data.sockid, NULL, NULL);
      fprintf(stderr,"AFTER mtcp_accept... with c = %d\n", c);
      if (c >= 0)
      {
        if (c >= MAX_FLOW_NUM)
        {
...
int main(int argc, char **argv)
{
  ...
  ret = mtcp_init("epwget-multiprocess.conf");
  if (ret)
  {
...
/*----------------------------------------------------------------------------*/
// sudo ./client_multicore send 192.168.10.118 8080 500 2